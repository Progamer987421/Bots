...
class BotManager {
  ...
  createBots(count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      let username;
      // Guarantee unique username across existing accounts
      ...
      this.meta[id] = {
        ...
        autoRejoin:       true,
        registered:       false,
        verificationKick: false, // true when kicked by bot-check, skip backoff
        inBanana:         false, // true after /server banana sent
        captchaPending:   false, // true when waiting to solve map captcha
      };
      this.logs[id] = [];
      this._spawnBot(id);
      created.push(id);
  ...
  _spawnBot(id) {
    ...
    this._log(id, `Connecting as ${username} → ${SERVER_HOST}:${SERVER_PORT}`);
    ...
    bot.on('kicked', (reason) => {
      ...
      if (isVerifyKick) {
        ...
        this.meta[id].verificationKick = false;
        this._log(id, 'Bot-check kick detected — fast rejoin in 2s');
        this._cleanup(id);
        if (this.meta[id]?.autoRejoin) {
          this.timers[id] = setTimeout(() => this._spawnBot(id), 2000);
        }
      } else {
    ...
    bot.on('end', (reason) => {
      ...
      // If inBanana is true this was a server-transfer disconnect — fast rejoin
      const wasBananaTransfer = this.meta[id]?.inBanana &&
        (Date.now() - (this.meta[id]._spawnTime || 0)) < 15000;

      this.meta[id].status   = wasBananaTransfer ? 'transferring — rejoining' : 'offline';
      this.meta[id].inBanana = false;
      this._cleanup(id);

      if (this.meta[id]?.autoRejoin) {
        if (wasBananaTransfer) {
          this._log(id, 'Server transfer disconnect — fast rejoin in 2s');
          this.timers[id] = setTimeout(() => this._spawnBot(id), 2000);
        } else {
          this._scheduleReconnect(id);
        }
